...
def parse_arguments(mode='train'):
    ...
    if mode == 'train':
        ...
        parser.add_argument('--iou_loss_weight', default=0.0, type=float,
                            help='To specify the regularization IoU loss weight.')
        parser.add_argument('--iou_loss_type', default='iou', type=str,
                            help='To specify the regularization IoU loss type.')
        ...
        parser.add_argument('--specify_epoch', default=None, nargs='+',
                            help='Select network weights with specific number of training epochs.')
        ...
        parser.add_argument('--test_pkl', type=str, default=None,
                            help='To specify specific test pkl file for sampling/evaluation.')
        parser.add_argument('--skip_eval', default=False, action='store_true',
                            help='To skip F1 score calculation to save time.')
        parser.add_argument('--random_node_num', default=False, action='store_true',
                            help='To randomly sample node number during evaluation.')
        args = parser.parse_args()
        ...
        if os.path.isfile(args.model_path):
            ...
            _epoch_str_ls = [os.path.basename(model_path).split('_')[-1].replace('.pth', '') for model_path in _model_path_ls]
            model_path_ls = []
            if args.specify_epoch is not None:
                # select ckpts based in the specified epochs
                args_specify_epoch = [int(item) for item in args.specify_epoch]
                for epoch_str, model_path in zip(_epoch_str_ls, _model_path_ls):
                    if epoch_str == 'best':
                        continue
                    if int(epoch_str) in args_specify_epoch:
                        model_path_ls.append(model_path)
            else:
                # select ckpts based on the range of epochs and the number of ckpts
    ...
    _sampling_overwrite_keywords_train_test = ['eval_size', 'batch_size']
    _sampling_overwrite_keywords_test = ['test_pkl', 'skip_eval', 'random_node_num']
    ...
    # add scene graph generation flag
    with config.unlocked():
        _scene_graph_keyword = ['visual_genome', 'coco_stuff']
        for _sg_key in _scene_graph_keyword:
            if _sg_key in config.dataset.name:
                config.flag_sg = True
                break
            else:
                config.flag_sg = False
...
def set_seed_and_logger(config, log_level, comment, dist_helper, eval_mode=False):
    ...
    if any([name in config.model.name for name in ['diffuse_sg']]):
    ...
    if eval_mode:
        str_eval = 'sample'
        str_eval += '_{:d}'.format(config.test.eval_size) if config.test.eval_size != 0 else '_full'
        str_eval += '_noEval' if config.test.skip_eval else ''
    else:
        str_eval = None
